...
def initMain():
	...
	for line in inputLines:
		...
		pairs.append((prod1_id, prod2_id, label))
...
def main():
	...
	print 'Label'
	label = dict()
	label.update({'MATCH':'1'})
	label.update({'MISMATCH':'0'})
	for pair in pairs:
		updateBrand(pair[0])
		updateBrand(pair[1])
		...
		print ' '.join(featureVector), label[pair[2]]
	return

if __name__ == '__main__':
	if len(sys.argv) < 2 or len(sys.argv) > 3:
		print >> sys.stderr, 'Usage: python cs784stage3.py <input> [<num_attributes>]'
		exit()
	global IS_DEBUG
	IS_DEBUG = False
	main()
