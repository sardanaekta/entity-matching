...
if len(sys.argv) != 3:
  ...
  print 'Usage: python _RF.py <input_file> <pos_thresh>'
  exit()
...
#Output filename
filename = 'RF_' + sys.argv[1][:sys.argv[1].rfind('.')]
...
X_train, X_test, y_train, y_test, i_train, i_test = train_test_split(X_data, y_data, range(0, len(y_data)), test_size=0.20)#, random_state=0)
